...
export function DeckTile({ count, active, label, onClick }: DeckTileProps) {
  return (
    <button
      ...
      disabled={!active}
      className={
        "flex flex-col items-center justify-between gap-1 w-[72px] h-[112px] rounded-md border px-1.5 py-1.5 text-[11px] transition " +
        (active
    ...
    >
      <span className="uppercase tracking-wider text-muted-foreground text-[11px]">Deck</span>
      <span
        className={
          "flex-1 flex items-center justify-center text-center " +
          (active
            ? "font-semibold text-[12px] leading-tight text-primary px-0.5"
