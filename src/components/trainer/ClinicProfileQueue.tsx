...
interface QueueRow {
  ...
  creator_type_count: number | null;
  signup_status: string | null;
}

function stage(r: QueueRow) {
  if ((r.creator_type_count ?? 0) > 0) return { label: "Profiled", cls: "border-forest/40 text-forest" };
  if ((r.photo_count ?? 0) > 0) return { label: "Ready to profile", cls: "border-primary/40 text-primary" };
...
export default function ClinicProfileQueue() {
              ...
              <div key={r.invitation_id} className="px-4 py-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    ...
                    {typeof r.photo_count === "number" ? ` • ${r.photo_count} photos` : ""}
                  </p>
                </div>
                ...
                {r.client_user_id && (
                  <Button asChild variant="outline" size="sm" className="h-7 text-xs flex-shrink-0">
                    <Link to={`/trainer/client/${r.client_user_id}`}>Open</Link>
                  </Button>
                )}
              </div>
            );
