...
interface ClientRecordingLinksProps {
  clientId: string;
  canEdit?: boolean;
}

export default function ClientRecordingLinks({ clientId, canEdit = true }: ClientRecordingLinksProps) {
  const { user } = useAuth();
  ...
  async function handleAdd() {
    ...
    setSaving(true);
    const { error } = await supabase.from("client_recordings").insert({
      client_id: clientId,
  ...
  async function handleDelete(id: string) {
    const { error } = await supabase.from("client_recordings").delete().eq("id", id);
    if (error) {
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
          <Video className="h-3.5 w-3.5" /> Session Recordings
        </p>
        {canEdit && !showForm && (
