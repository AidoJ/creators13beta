CREATE OR REPLACE FUNCTION public.finalise_ranked_match(_match_id uuid, _reason text DEFAULT 'normal'::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF _row.guest_user_id IS NULL OR _row.host_user_id IS NULL THEN
    -- Can't be scored (e.g. invite nobody accepted). Mark terminal so the
    -- ranked backstop stops retrying it forever.
...
CREATE OR REPLACE FUNCTION public.finalise_ranked_match(_match_id uuid, _reason text DEFAULT 'normal'::text, _placements jsonb DEFAULT NULL::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
  ...
  IF _player_count <= 2 THEN
    ...
    BEGIN
      IF _row.host_user_id IS NULL OR _row.guest_user_id IS NULL THEN
        -- Can't be scored (e.g. invite nobody accepted, auto-closed by the
        -- abandoned-match reaper). Mirror the N-player "fewer than 2 ranked
        -- entries" branch: mark terminal so the backstop stops retrying.
        UPDATE public.game_matches
           SET state = state || jsonb_build_object('__finalised', true)
         WHERE id = _match_id;
        RETURN;
